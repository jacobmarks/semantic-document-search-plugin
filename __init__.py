...
def _run_query(ctx):
    ...
    current_ids = ctx.view.values(
        f"{detection_field}.detections.id", 
        unwind=True
        )
    
    _filter = qmodels.Filter(
        must=[
            qmodels.HasIdCondition(has_id=_to_qdrant_ids(current_ids))
        ]
    )
...
class FuzzySearch(foo.Operator):
    ...
    def resolve_input(self, ctx):
        ...
        if len(valid_collections) == 0:
            ...
            detection_fields = [_extract_detections_field(c) for c in valid_collections]
            text_fields = [_get_text_field_name(ctx.dataset, df) for df in detection_fields]

            collection_choices = types.Dropdown(multiple=False)

            for cn, df, tf in zip(valid_collections, detection_fields, text_fields):
